...
@Component({
  selector: 'app-mario-barf',
  standalone: true,
  imports: [RouterLink, BreadcrumbsComponent, DownloadCtaComponent, BlogCardComponent],
  templateUrl: 'mario-barf.component.html',
  styleUrls: ['../../../juridisch/juridisch.component.scss', '../../blog-article-shared.scss', './mario-barf.component.scss']
})
export class MarioBarfComponent {
  private readonly post = BLOG_POSTS.find(p => p.slug === 'mario-barf')!;
  ...
  constructor(meta: Meta, @Inject(DOCUMENT) document: Document) {
    const title = 'Mario Barf - Spelregels & Tips | AnyTimerApp';
    const description = 'Mario Barf is Mario Kart met een drankje naast je controller. Ontdek de spelregels, benodigdheden en tips voor dit chaotische drankspel.';
    const image = 'https://anytimer.app/assets/blogs/drankspellen/mario%20barf.png';
    const url = 'https://anytimer.app/blog/drankspelletjes/mario-barf/';
    const publishedTime = '2026-07-24';

    injectBreadcrumbSchema(document, 'schema-breadcrumbs-mario-barf', this.breadcrumbs);
    ...
    meta.updateTag({ property: 'og:image:alt', content: 'Mario Barf drankspel met Mario Kart en drankjes naast de controller' });
    ...
    if (!document.getElementById('schema-mario-barf')) {
      const script = document.createElement('script');
      script.id = 'schema-mario-barf';
